...
// Linux-sempai says use only red, or red and cyan, and don't use the others;
// you have no idea whether they're using a dark or light theme, or what their
// theme is like and some colors WILL NOT show up, depending.
const ansiTags = {
	// styles
	'bold': '\u001B[1m', // aka bright
	'dim': '\u001B[2m', // aka dim
...
const ansiWrapBodge = arr => {
	...
	const bodged = arr.map(line=>{
		let prevWrap = wrappedTags.size ? [...wrappedTags].join('') : '';
		let pos = 0;
		...
		return prevWrap + line;
	});
	return bodged;
};
const buildSerialDialogFromInfo = (f, info, node) => {
	const serialDialog = {
		wrap: 80,
		...f.settings.serial || {}, // global settings
		...info.settings, // local settings
	};
	const wrappedMessages = info.messages
		.map(tagsToAnsiEscapes)
		.map(message=>wrapText(message, serialDialog.wrap, true))
	serialDialog.messages = wrappedMessages;
	if (info.options.length > 0) {
